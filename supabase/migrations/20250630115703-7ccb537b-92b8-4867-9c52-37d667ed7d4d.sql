...
-- First, let's check what columns exist in the user_roles table
...
-- Recreate the user role enum to ensure it exists
DROP TYPE IF EXISTS public.user_role CASCADE;
CREATE TYPE public.user_role AS ENUM ('Admin', 'Sales Manager', 'Sales Associate');

-- Add the role column if it doesn't exist, or alter it if it does
DO $$ 
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE public.user_roles ADD COLUMN role user_role NOT NULL DEFAULT 'Sales Associate';
    ELSE
        ALTER TABLE public.user_roles ALTER COLUMN role TYPE user_role USING 
            CASE 
                WHEN role::text = 'Admin' THEN 'Admin'::user_role
                WHEN role::text = 'Sales Manager' THEN 'Sales Manager'::user_role
                WHEN role::text = 'Sales Associate' THEN 'Sales Associate'::user_role
                ELSE 'Sales Associate'::user_role
            END;
    END IF;
END $$;
...
-- Update the get_user_role function to handle the correct enum type
